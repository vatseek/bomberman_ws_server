...
var Field = class {
    ...
    next() {
        //Move mobs
        for (var item in this.mobsList) {
            var mobPos = this.mobsList[item].getPosition();
            this.setPosition(mobPos[0], mobPos[1], g.NONE);
            this.mobsList[item].next();
            mobPos = this.mobsList[item].getPosition();
            this.setPosition(mobPos[0], mobPos[1], g.MEAT_CHOPPER);
        }

        for (item in this.bombsList) {
            this.bombsList[item].next();
        }
    }
};
